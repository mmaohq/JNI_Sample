...
class SelectIOBase_mac : virtual public NZBufferedIO
{
protected:
    int fd;
...
class NZComIO_mac : public SelectIOBase_mac, public NZComIO
{
public:
    bool Open(const char *name, const unsigned int baudrate, const ComDataBits databits, const ComParity parity, const ComStopBits stopbits, const ComFlowControl flowcontrol)
    {
        ...
        // set the baudrate
        unsigned int baudrates[] = { 9600, 19200, 38400, 57600, 115200, 230400 };
        unsigned int baudflags[] = {B9600,B19200,B38400,B57600,B115200,B230400 };
        ...
        // choosing raw output
        tios.c_oflag &= ~(OPOST | ONLCR | OCRNL | ONOCR | ONLRET | OFILL);
...
class NZUsbIO_mac : public SelectIOBase_mac, public NZUsbIO
{
public:
...
class NZLptIO_mac : public SelectIOBase_mac, public NZLptIO
{
public:
    bool Open(const char *name)
    {
...
class NZPrnIO_mac : public NZPrnIO
{
public:

};

class NZSocketIO_mac : virtual public NZSocketIO
{
protected:
    int s;
...
class NZTcpClientIO_mac : public NZSocketIO_mac, public NZTcpClientIO
{
public:
    ...
    bool setsockopt_keepintvl(const unsigned int keepintvl)
    {
        ...
        return setsockopt(s, IPPROTO_TCP, TCP_KEEPINTVL, &val, sizeof(val)) == 0;
    }

    bool setsockopt_keepcnt(const unsigned int keepcnt)
    {
        int val = (int)keepcnt;
        return setsockopt(s, IPPROTO_TCP, TCP_KEEPCNT, &val, sizeof(val)) == 0;
    }
...
class NZUdpClientIO_mac : public NZSocketIO_mac, public NZUdpClientIO
{
public:
...
class NZUsbIOEnumerator_mac : public NZUsbIOEnumerator
{
public:
    virtual size_t Enumerate(char *buffer, size_t length, size_t *prequired)
    {
        const char *prefix[] = { "lp", 0, };
        return NZDirectoryList_mac::ListFiles("/dev/usb", prefix, buffer, length, prequired);
    }
};

class NZComIOEnumerator_mac : public NZComIOEnumerator
{
public:
    virtual size_t Enumerate(char *buffer, size_t length, size_t *prequired)
    {
        const char *prefix[] = { "ttyS", "ttyUSB", 0, };
        return NZDirectoryList_mac::ListFiles("/dev", prefix, buffer, length, prequired);
    }
};

class NZLptIOEnumerator_mac : public NZLptIOEnumerator
{
public:
    virtual size_t Enumerate(char *buffer, size_t length, size_t *prequired)
    {
...
class NZPrnIOEnumerator_mac : public NZPrnIOEnumerator
{
public:

};

class NZHostIPEnumerator_mac : public NZHostIPEnumerator
{
public:
    virtual size_t Enumerate(char *buffer, size_t length, size_t *prequired)
    {
